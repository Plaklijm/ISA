...
void UISACharacterMovementComponent::PhysSlide(float deltaTime, int32 Iterations)
{
	...
	while ( (remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) && CharacterOwner && (CharacterOwner->Controller || bRunPhysicsWithNoController || (CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy)) )
	{
		...
		else
		{
			...
			{
				GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red, TEXT("Need to start Falling"));
	...
	if (!CanSlide())
	{
		UE_LOG(LogTemp, Error, TEXT("cant Slide"));
		SetMovementMode(MOVE_Walking);
		StartNewPhysics(deltaTime, Iterations);
		return;
	}
	...
	bool bCheckedFall = false;
	bool bTriedLedgeMove = false;
	UE_LOG(LogTemp, Error, TEXT("perform the move"));#1#
	// Perform the move
	...
	//float remainingTime = deltaTime;
		...
		bJustTeleported = false;
		/*const float timeTick = GetSimulationTimeStep(remainingTime, Iterations);
		remainingTime -= timeTick;#1#

		auto FloorInfluence = CalculateFloorInfluence(CurrentFloor.HitResult.Normal) * FloorInfluenceFactor;  
		AddForce(FloorInfluence);

		if (Velocity.Size() > Settings->SprintSpeed)
		{
			UE_LOG(LogTemp, Error, TEXT("SprintSpeed"));
			Velocity = Velocity.GetSafeNormal() * Settings->SprintSpeed;
		}

		if (Velocity.Size() < Settings->CrouchSpeed)
		{
			UE_LOG(LogTemp, Error, TEXT("CoruchSpeed"));
			return;
			//ExitSlide();
		}
	...
	*/
	...
	while ((remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) && CharacterOwner && (CharacterOwner->Controller || bRunPhysicsWithNoController || (CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy)))
	{
		...
		else
		{
			...
			if (IsFalling())
			{
				UE_LOG(LogTemp, Error, TEXT("Is Falling"));
				// pawn decided to jump up
				const float DesiredDist = Delta.Size();
				if (DesiredDist > KINDA_SMALL_NUMBER)
				{
		...
		{
			UE_LOG(LogTemp, Error, TEXT("Floor check"));
			// Validate the floor check
			if (CurrentFloor.IsWalkableFloor())
			{
				if (ShouldCatchAir(OldFloor, CurrentFloor))
